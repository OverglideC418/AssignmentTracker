...
const deviceId = (() => {
  const key = 'assignmenttracker-device-id'
...
function futureMatchingDates(item: Assignment, allItems: Assignment[], timezone: string) {
  if (item.kind !== 'imported' || item.source_id == null) return []
  const currentDue = dateOnly(item.due_at, timezone)
  return [...new Set(allItems
    .filter((candidate) => candidate.kind === 'imported' && candidate.id !== item.id && candidate.source_id === item.source_id && normalizedTitle(candidate.title) === normalizedTitle(item.title))
    .map((candidate) => dateOnly(candidate.due_at, timezone))
    .filter((candidateDate) => candidateDate > currentDue))]
    .sort()
}
...
function Settings({ sources, preferences, onClose, onRefresh }: { sources: Source[]; preferences: Record<string, unknown>; onClose: () => void; onRefresh: () => Promise<void> }) {
  ...
  async function exportBackup() { try { const response = await fetch('/api/export', { credentials: 'include' }); if (!response.ok) throw new Error('Could not export backup'); const blob = await response.blob(); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `assignmenttracker-backup-${new Date().toISOString().slice(0, 10)}.json`; link.click(); URL.revokeObjectURL(link.href); setMessage('Backup exported. Store it somewhere secure because it contains calendar URLs.') } catch (err) { setMessage((err as Error).message) } }
  async function importBackup(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; event.target.value = ''; if (!file || !window.confirm('Restore this backup? Current sources, assignments, custom tasks, and preferences will be replaced.')) return; try { const payload = JSON.parse(await file.text()); const result = await api<{ sources: number; assignments: number; custom_tasks: number }>('/api/import', { method: 'POST', body: JSON.stringify(payload) }); setMessage(`Restored ${result.sources} sources, ${result.assignments} assignments, and ${result.custom_tasks} custom tasks.`); await onRefresh() } catch (err) { setMessage((err as Error).message) } }
...
export default function App() {
  ...
  if (setup === null) return <main className="auth-page"><section className="auth-card"><div className="brand-mark">A</div><p className="eyebrow">PRIVATE ASSIGNMENT TRACKER</p><h1>Loading AssignmentTracker</h1><p className="muted">Connecting to your private server…</p></section></main>
